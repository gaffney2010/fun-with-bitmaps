...
Canvas::Canvas(int width, int height) {
  canvas = std::vector<std::vector<RgbColor>>(width, std::vector<RgbColor>(height, {0, 0, 0}));
...
Canvas::Canvas(const Dimensions &dim) {
  canvas = std::vector<std::vector<RgbColor>>(dim.w, std::vector<RgbColor>(dim.h, {0, 0, 0}));
  this->dim = dim;
...
RgbColor Canvas::get(int x, int y) const {
  return canvas[x][y];
}

RgbColor Canvas::get(const Coord &coord) const { return canvas[coord.x][coord.y]; }
